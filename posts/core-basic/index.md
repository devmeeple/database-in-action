...
- 이전에 학습한 조인(`INNER`, `OUTER`, `SELF`)은 이미 맺어진 관계를 연결하는 방법이다. 하지만 `CROSS JOIN`은 연결고리 없이, 한쪽 테이블의 모든 행을 다른 쪽 테이블의 모든 행과
  하나씩 전부 연결하는 방법이다.
    - 연결고리가 없기 때문에 `ON`절을 사용하지 않는다.
- `CROSS JOIN`은 수학 용어로 카테시안 곱(Cartesian Product)라고 부른다. A 테이블의 행이 `m`개, B 테이블의 행이 `n`개라면 두 테이블의 `CROSS JOIN` 결과는 `m * n`
  의 행이다.
- `CROSS JOIN`은 모든 경우의 수를 만들기 때문에 유용하지만 실무에서 사용할 때는 주의가 필요하다. 행 결과가 기하급수적으로 늘어나 데이터베이스 서버가 멈출 수 있다.

### INSERT INTO ... SELECT

- `INSERT INTO ... SELECT` 구문은 `SELECT`문으로 조회된 결과를 즉시 다른 테이블에 삽입하는 기능이다. 주로 데이터를 한 번에 대량으로 삽입할 때 사용한다.

## 3.5 조인 종합 실습

문제 해결은 '문장 잘게 나누기'부터 시작이다.
